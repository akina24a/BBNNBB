...
Varyings ShadowPassVertex(Attributes input, uint instanceID: SV_InstanceID)
{
	...
	VertexOutput vertexData = GetShadowVertexOutput(instanceID,vertexInputs, posOffset, wind, AO_MASK);
...
half4 ShadowPassFragment(Varyings input) : SV_TARGET
{
	...
	AlphaClip(alpha, _ShadowCutoff, input.positionCS.xyz, input.positionWS.xyz );
